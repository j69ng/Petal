...
import type { Draft, PaymentKind, Purchase, RecordStatus } from "./types";

/**
 * Loads the sample books. Used by `npm run seed` and, on a demo deployment, on
 * first boot — one copy of this data rather than two that drift apart.
 */
export function seedSampleData(log: (message: string) => void = () => {}): void {
  const database = db();
  ...
  log("Seeded:");
  log(`  ${houseALines.length + houseBLines.length} material purchases across 2 builds`);
  log(`  ${people.length + 1} workers, attendance for both builds, wages and advances`);
  ...
  log(`  usual prices for ${USUAL.length} materials`);
  log(`  ${SUPPLIERS.length} suppliers and ${QUOTES.length} rates in the book`);
  log("");
  log("Sign in as the owner, then:");
  log("  /approvals  — the entries waiting to be confirmed before they count");
  log("  /compare    — the second build measured against the first");
  log("  /vendors    — who sells the same materials cheaper, and by how much");
